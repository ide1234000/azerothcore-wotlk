...
public:
    ...
    LearnSpellsOnLevelUp() : PlayerScript("LearnSpellsOnLevelUp")
    {
        uint32 temp[] =
        {
		 ...
		 20647, 23455, 62900, 42243, 24131, 49376, 16979, 54055,
		 18848, 65878, 58433, 58432, 42234, 42245, 42244, 48075,
		 25329, 27805, 27804, 27803, 23459, 23458, 66907, 66906,
		 50589, 59671, 62902, 62901, 48076, 
        };
    ...
    void OnLogin(Player *player)
    {
        if (sConfigMgr->GetBoolDefault("LearnSpells.Enable", true))
        {
            ...
            {
                ChatHandler(player->GetSession()).SendSysMessage("所有角色技能在升级后将 |cff4CFF00自动学习|r！");
            }
        }
    }
