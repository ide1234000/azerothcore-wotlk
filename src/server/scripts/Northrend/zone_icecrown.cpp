        ...
        void CheckSummons()
        {
            ...
            if (allow)
            {
                ...
                if (Player* player = ObjectAccessor::GetPlayer(*me, playerGUID))
                {
                    ...
                    {
                        case QUEST_BFV_FALLEN_HEROES:
                            me->MonsterYell("$N 已经击败了过去的瓦哈拉战役的堕落英雄。 这只是一个开始，但它就足够了。", LANG_UNIVERSAL, ObjectAccessor::GetPlayer(*me, playerGUID));
                            break;
                        case QUEST_BFV_DARK_MASTER:
                            me->MonsterYell("黑暗大师希特里克斯已经被 $N 和他的同伴们打败了。让下一个挑战开始吧！", LANG_UNIVERSAL, ObjectAccessor::GetPlayer(*me, playerGUID));
                            break;
                        case QUEST_BFV_SIGRID:
            ...
            switch (events.GetEvent())
            {
                case EVENT_VALHALAS_FIRST:
                {
                    switch (currentQuest)
                    {
                        ...
                        case QUEST_BFV_THANE:
                            events.ScheduleEvent(EVENT_VALHALAS_SECOND, 8000);
                            me->MonsterYell("死亡之锤巴纳霍格领主回到了瓦哈拉，以证明弗里库是唯一值得在其神圣之环内战斗的生物。美元能证明他是错的吗？", LANG_UNIVERSAL, ObjectAccessor::GetPlayer(*me, playerGUID));
                            break;
                        case QUEST_BFV_FINAL:
                            events.ScheduleEvent(EVENT_VALHALAS_SECOND, 8000);
                ...
                case EVENT_VALHALAS_SECOND:
                {
                    switch (currentQuest)
                    {
                        case QUEST_BFV_FALLEN_HEROES:
                            ...
                            break;
                        case QUEST_BFV_DARK_MASTER:
                            me->MonsterTextEmote("黑魔王希特里克斯从东南部滑向瓦哈拉！", NULL, true);
                            break;
                        case QUEST_BFV_CARNAGE:
                        ...
                        case QUEST_BFV_THANE:
                            ...
                            break;
                        case QUEST_BFV_FINAL:
                            me->MonsterYell("约敦海姆的战士们，我向你们呈现，血王子桑多瓦尔！", LANG_UNIVERSAL, NULL);
