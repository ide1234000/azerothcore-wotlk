        ...
        if (canStart)
        {
            ...
            if (status == QUEST_STATUS_COMPLETE || status == QUEST_STATUS_REWARDED)
            {
                if (creature->GetEntry() == NPC_SYLVANAS_PART1)
                    AddGossipItemFor(player, 0, "黑暗女士，我想我听到阿尔萨斯来了。无论你要做什么，都要快点做。", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
                else
                    AddGossipItemFor(player, 0, "夫人，我想我听到阿尔萨斯来了。无论你要做什么，都要快点。", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
            }
        }
